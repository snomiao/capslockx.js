...
const model2D = AccModel2D((dx = 0, dy = 0) => {
    position.x += dx;
    position.y += dy;
...
// initial moving,
setTimeout(() => {
    pointerControl.right.press(); pointerControl.start()
    pointerControl.down.press(); pointerControl.start()
}, 0)
setTimeout(() => {
...
window.addEventListener('keydown', (event) => {
    switch (event.key) {
        case 'ArrowLeft':
        case 'a':
            pointerControl.left.press(); pointerControl.start()
            break;
        case 'ArrowRight':
        case 'd':
            pointerControl.right.press(); pointerControl.start()
        ...
        case 'ArrowUp':
        case 'w':
            pointerControl.up.press(); pointerControl.start()
            break;
        case 'ArrowDown':
        case 's':
            pointerControl.down.press(); pointerControl.start()
